...
You have access to live data tools that read THIS workspace's reservations, listings, pricing, and guest CRM. Use them. NEVER guess numbers, dates, or guest details — call the tool.
...
export function createAgentHandler(opts: AgentHandlerOptions): (req: Request) => Promise<Response> {
  ...
  return async function handler(req: Request): Promise<Response> {
    ...
    if (fb.ok) {
      return wrappedStreamResponse(fb.chunks, fb.iter, fb.errBox, {
